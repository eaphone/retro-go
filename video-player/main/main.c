...
int mjpeg_play(const char *filepath)
{
    RG_LOGI("mjpeg_play is called");
    ...
    uint8_t *work_buf = (uint8_t*)heap_caps_malloc(work_size, MALLOC_CAP_INTERNAL);
    if (!work_buf) {
        free(frame_buf);
        avi_player_close(&player);
        return -1;
    ...
    rg_surface_t *currentUpdate = updates[0];
